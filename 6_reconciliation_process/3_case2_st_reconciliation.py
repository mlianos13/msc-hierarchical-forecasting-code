...
reconciliation_data[(5, 48)] = res48_armax[("HC.f", 1)]  # No reindexing/padding
...
for k in range(1, 3):  # k = 1, 2
    ...
    if k == 1:
        ...
        col_name = ("", k)
    reconciliation_data[(4, horizon)] = res24_armax[col_name]  # No reindexing
...
for k in range(1, 5):  # k = 1, 2, 3, 4
    ...
    if k == 1:
        ...
        col_name = ("", k)
    reconciliation_data[(3, horizon)] = res12_armax[col_name]  # No reindexing
...
for k in range(1, 9):  # k = 1,...,8
    ...
    if k == 1:
        ...
        col_name = ("", k)
    reconciliation_data[(2, horizon)] = res6_armax[col_name]  # No reindexing
...
for k in range(1, 25):
    ...
    if k == 1:
        ...
        col_name = ("", k)
    reconciliation_data[(1, horizon)] = res2_rls[col_name]  # No reindexing
...
for k in range(1, 49):
    ...
    if k == 1:
        ...
        col_name = ("", k)
    reconciliation_data[(0, horizon)] = res1_rls[col_name]  # No reindexing
...
res1_rls_test = res1_rls.iloc[len(res1_rls) - len(obs_test):].reset_index(drop=True)
...
print(f"res1_rls_test shape: {res1_rls_test.shape}")
...
for h in range(1, rec_res1_test.shape[1] + 1):
    ...
    forecasts_base = res1_rls_test.iloc[:, idx]
    ...
    obs_time_axis      = np.arange(len(obs_test))
    valid_len          = N - h
    ...
    ax.plot(forecast_time_axis, valid_rec.values,
            label='Reconciled', color='slateblue', linewidth=1.8, zorder=1)

    # Plot only valid base forecasts
    ax.plot(forecast_time_axis, valid_base.values,
            label='RLS',        color='orange',   linewidth=1.8, zorder=2)

    ax.set_title(f'Forecast Comparison at Horizon {h}', fontsize=12)
    ax.set_xlabel('Time Step (t)',             fontsize=12)
    ax.set_ylabel('Heat Load - Case 2',        fontsize=12)
    ax.legend(loc='best')
    ax.grid(False)
...
# Save results
comparison_df.to_csv('rmse_case2_st_2.csv')
