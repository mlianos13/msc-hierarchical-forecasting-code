# process_svr_12h.py (Direct Multi-step SVR for 12-hour aggregated data)
#%%
import sys
import os
...
# --- 3. Aggregate to 12-hourly features & target ---
twelve_hourly = c.new_fc()

# target: sum of the *next* 12 hours of HC.f
twelve_hourly[('HC.f_future12h','NA')] = (
    hourly_data[('HC.f','NA')]
    .shift(-12)
...
# autoregressive feature: sum of the *past* 12 hours
twelve_hourly[('HC.f_past12h','NA')] = (
    hourly_data[('HC.f','NA')]
    .rolling(window=12, min_periods=12)
    .sum()
)

# smoothed exogenous features: 12-h rolling mean
for var in ['Taobs','W.f','GR.f']:
    twelve_hourly[(f"{var}_smooth",'NA')] = (
        hourly_data[(var,'NA')]
        .rolling(window=12, min_periods=12)
        .mean()
    )
...
twelve_hourly.dropna(inplace=True)

# --- 4. SVR Hyperparameters (same as 2h/6h versions) ---
...
# --- 5. Direct multi-step SVR over 12-hour series ---
horizons_to_predict = list(range(1, 5))  # 1→12h, …, 4→48h
max_lag = 48  # keep last 48 twelve-hour blocks for the rolling window

all_preds = []
print("Starting direct multi-step SVR 12-hour training & prediction...")
for h in horizons_to_predict:
    print(f"  Horizon {h} (forecast {12*h}h ahead)…")
    model = ForecastModel(
        OnlineSVR,
    ...
    model.add_outputs('HC.f_future12h')
    model.add_inputs(
        ar_0    = AR('HC.f_past12h', order=0),
        ar_1    = AR('HC.f_past12h', order=1),
        ar_2    = AR('HC.f_past12h', order=2),
        ar_3    = AR('HC.f_past12h', order=3),
        ar_4    = AR('HC.f_past12h', order=4),
    ...
    preds = []
    # slide a 1-row window through twelve_hourly
    for i in range(len(twelve_hourly) - h):
        batch = twelve_hourly.iloc[i:i+1].copy()
        future_val = twelve_hourly[('HC.f_future12h','NA')].iloc[i+h]
        if np.isnan(future_val):
            continue
        # inject true future for the online update
        batch[('HC.f_future12h','NA')] = future_val

        out = model.update(batch, n_keep=max_lag)
        df_out = out[0] if isinstance(out, tuple) else out
        if isinstance(df_out, pd.DataFrame) and not df_out.empty:
            val = df_out.iloc[0][('HC.f_future12h', h)]
            ts = pd.Series(
                [val],
                index=[twelve_hourly.index[i+h]],
                name=('HC.f_future12h', h)
            )
            preds.append(ts)
...
def unscale_sum(x, mn, mx, n=12):
    # for sums of n scaled values: x*(mx-mn) + n*mn
    return x * (mx - mn) + (n * mn)

plot_dir = "../7_results/Case_2/3_svr/res12"
os.makedirs(plot_dir, exist_ok=True)

# --- Predictions (h=12) ---
preds_unscaled = unscale_sum(combined_pred, hc_f_min, hc_f_max, n=12)
...
preds_unscaled.to_csv(
    os.path.join(plot_dir, "svr_res12h.csv"),
    index_label=['', '']    # blank out the top‐left cell
)
print(f"Predictions saved → {os.path.join(plot_dir, 'svr_res12h.csv')}")

# --- Observations (NA‐horizon) ---
obs_scaled   = twelve_hourly[('HC.f_future12h','NA')]
obs_unscaled = unscale_sum(obs_scaled, hc_f_min, hc_f_max, n=12).to_frame()
...
obs_unscaled.to_csv(
    os.path.join(plot_dir, "observations_HC_12h.csv"),
    index_label=['', '']
)
print(f"Observations saved → {os.path.join(plot_dir, 'observations_HC_12h.csv')}")

# --- Combined (obs + preds) ---
combined_df = pd.concat([obs_unscaled, preds_unscaled], axis=1)
combined_df.to_csv(
    os.path.join(plot_dir, "svr_res12h_with_obs.csv"),
    index_label=['', '']
)
print(f"Combined saved → {os.path.join(plot_dir, 'svr_res12h_with_obs.csv')}")

# --- 7. Evaluation & Plotting for first 12h horizon (h=1) ---
burn_in = 48  # skip first 48 twelve-hour steps for metrics
for var, h in combined_pred.columns:
    if var != 'HC.f_future12h':
        continue
    pred_s = combined_pred[(var, h)].dropna()
    if pred_s.empty:
        continue

    y_pred = unscale_sum(pred_s, hc_f_min, hc_f_max, n=12)
    y_obs  = unscale_sum(obs_scaled, hc_f_min, hc_f_max, n=12)
    ...
    print(f"Horizon {h} (12h ahead) → RMSE={rmse:.4f}, MAPE={mape:.2f}%, R2={r2v:.4f}")

    if h == 1:
        # forecast vs actual
        fig, ax = plt.subplots(figsize=(16, 4))
        ax.plot(y_true.index, y_true, label='Actual 12h Sum',   linewidth=2.0)
        ax.plot(y_hat.index, y_hat, label='Predicted 12h Sum', linewidth=0.8)
        ax.set_title("SVR 12-Hour Forecast (h=1) – Post Burn-in")
        ax.legend(); ax.grid(True)
        fig.savefig(
            os.path.join(plot_dir, "svr_forecast_12h_1_step_post_burn_in.png"),
        ...
        plot_acf(res, lags=min(8, len(res)-1), ax=ax1); ax1.set_title("ACF of Residuals")
        plot_pacf(res, lags=min(8, len(res)//2-1), ax=ax2); ax2.set_title("PACF of Residuals")
        qqplot(res, line="s", ax=ax3); ax3.set_title("Q-Q Plot of Residuals")
        ax4.plot(res.index, res); ax4.set_title("Residuals Over Time")
        plt.tight_layout()
        fig.savefig(
            os.path.join(plot_dir, "svr_diagnostics_12h_1_step.png"),
            dpi=300, bbox_inches='tight'
        )
        plt.close(fig)
