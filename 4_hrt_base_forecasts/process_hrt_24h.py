...
# 2. Aggregate to 24-hourly (daily) data
twenty_four_hourly_data = c.new_fc()

# Target variable: Sum of the next 24 hours of HC.f (what we want to predict)
sum_target_24h = hourly_data[("HC.f", "NA")].shift(-24).rolling(window=24, min_periods=24).sum()
twenty_four_hourly_data[("HC.f_future24h", "NA")] = sum_target_24h

# Input feature: Sum of the past 24 hours of HC.f (for autoregressive terms)
...
# Input feature: Smoothed average of the past 24 hours of W.f
twenty_four_hourly_data[("W.f_smooth", "NA")] = hourly_data[("W.f", "NA")].rolling(window=24, min_periods=24).mean()
# Input feature: Smoothed average of the past 24 hours of GR.f
twenty_four_hourly_data[("GR.f_smooth", "NA")] = hourly_data[("GR.f", "NA")].rolling(window=24, min_periods=24).mean()

# Remove rows with NaN values resulting from rolling window operations
twenty_four_hourly_data.dropna(inplace=True)


# --- ARFR Model Setup for 24-hour data ---
...
# Horizons for 24-hour steps (k=1 is a 1-day ahead forecast)
model24 = ForecastModel(ARFR, estimate_variance=False, kseq=tuple(range(1, 3)))
model24.add_outputs("HC.f_future24h") # Predict the 24-hour sum
model24.add_inputs(
    # Autoregressive terms on the 24-hour sum
    ar_0=AR("HC.f_past24h", order=0),
    # Daily lag is 1 step for 24-hour data
    ar_1=AR("HC.f_past24h", order=1),
    # Exogenous variables on smoothed 24-hour data
...
res24_scaled = model24.fit(twenty_four_hourly_data)
print("ARFR 24h model fitting completed.")


# --- Unscale and Save Results ---

# 1. Create a directory for plots and results
plot_dir = "../7_results/Case_1/4_ohdt/res24"
...
def unscale_sum_24h(series_or_df, mn, mx):
    """
    Unscales a pandas Series or DataFrame that represents a sum of twenty-four scaled values.
    Derivation:
    y_sum_unscaled = y1_unscaled + ... + y24_unscaled
                   = (y1_scaled * (mx - mn) + mn) + ... + (y24_scaled * (mx - mn) + mn)
                   = (y1_scaled + ... + y24_scaled) * (mx - mn) + 24 * mn
                   = y_sum_scaled * (mx - mn) + 24 * mn
    """
    return series_or_df * (mx - mn) + (24 * mn)

hc_f_min = orig_min['HC.f']
hc_f_max = orig_max['HC.f']

# 3. Get scaled predictions and observations
if isinstance(res24_scaled, tuple) and len(res24_scaled) > 0 and isinstance(res24_scaled[0], pd.DataFrame):
    predictions_scaled_df = res24_scaled[0]
elif isinstance(res24_scaled, pd.DataFrame):
    predictions_scaled_df = res24_scaled
else:
    raise RuntimeError("Unexpected format for res24_scaled from model24.fit")

observations_scaled_df = twenty_four_hourly_data.fc[["HC.f_future24h"]].loc[predictions_scaled_df.index]

# 4. Unscale the data using the correct formula for sums
predictions_unscaled_df = unscale_sum_24h(predictions_scaled_df, hc_f_min, hc_f_max)
observations_unscaled_df = unscale_sum_24h(observations_scaled_df, hc_f_min, hc_f_max)

# 5. Save the unscaled predictions, observations, and combined file
out_path_preds = os.path.join(plot_dir, "arfr_res24h.csv")
...
combined_path = os.path.join(plot_dir, "arfr_res24h_with_obs.csv")
...
# 6. Prepare DataFrame for evaluation (using horizon k=1, which is a 24-hour ahead forecast)
horizon = 1
comparison_df = pd.DataFrame({
    'Obs': combined_unscaled_df[('HC.f_future24h', 'NA')],
    'Pred': combined_unscaled_df[('HC.f_future24h', horizon)]
})

# 7. Calculate RMSE and get data after a burn-in period
burn_in = 4  # Define a burn-in period (e.g., 4 * 24h steps = 96 hours)
valid_df = comparison_df.iloc[burn_in:].dropna()

if not valid_df.empty:
    residuals = valid_df["Obs"] - valid_df["Pred"]
    rmse = np.sqrt((residuals ** 2).mean())
    print(f"\nHorizon {horizon} (24 hours) -> RMSE (post burn-in): {rmse:.4f}")
else:
    print(f"\nHorizon {horizon} -> No valid data for RMSE computation post burn-in.")

# 8. Plot forecast vs. actual (POST BURN-IN)
fig, ax = plt.subplots(figsize=(16, 4))
ax.plot(valid_df.index, valid_df["Obs"], label="Actual 24h Sum (Unscaled)", color="black", linewidth=2.0)
ax.plot(valid_df.index, valid_df["Pred"], label=f"Predicted 24h Sum (Unscaled, k={horizon})", color="blue", linestyle="-", linewidth=0.8)
ax.set_title(f"Online ARFR 24-Hour Forecast (k={horizon}) - Post Burn-in")
ax.legend()
ax.grid(True)
plt.tight_layout()
fig_path = os.path.join(plot_dir, f"arfr_forecast_24h_{horizon}_step_post_burn_in.png")
...
if not valid_df.empty:
    ...
    # ACF Plot (2 lags represent 48 hours)
    plot_acf(residuals, lags=min(2, len(residuals)-1), ax=ax1)
    ax1.set_title("ACF of Residuals (Post Burn-in)")
    
    # PACF Plot
    plot_pacf(residuals, lags=min(2, max(0, len(residuals)//2 - 1)), ax=ax2)
    ...
    plt.tight_layout()
    diag_fig_path = os.path.join(plot_dir, f"arfr_diagnostics_24h_{horizon}_step.png")
