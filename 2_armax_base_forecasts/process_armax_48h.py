...
#
# 1. Prepare 48-hourly aggregated data
#
...
# Create the final DataFrame for the 48-hour model
forty_eight_hourly_data = c.new_fc()

# Feature Engineering: Create rolling features for 48-hour aggregation
# Input feature: Sum of the PAST 48 hours of HC.f
...
# Target variable: Sum of the NEXT 48 hours of HC.f
...
armax_exog_feature_names = [
    'HC.f_past48h', 'Taobs_smooth'
]

# Initialize the ForecastModel with the ARMAX predictor
model_armax_48h = ForecastModel(
    ARMAX,
    ...
    kseq=tuple(range(1, 2)) # Predict 1 step ahead (1 * 48h = 48h forecast horizon)
)

# Define model output and inputs
model_armax_48h.add_outputs("HC.f_future48h")
model_armax_48h.add_inputs(
    "HC.f_past48h",
    "Taobs_smooth"
)

# Fit the model
res48h = model_armax_48h.fit(forty_eight_hourly_data.copy())
print("ARMAX 48h model fitting completed.")

# --- Save Raw Predictions to CSV ---
plot_dir = "../7_results/Case_1/2_armax/res48"
os.makedirs(plot_dir, exist_ok=True)

# Ensure predictions are in the expected DataFrame format
if isinstance(res48h, tuple) and len(res48h) > 0 and isinstance(res48h[0], pd.DataFrame):
    predictions_df = res48h[0]
elif isinstance(res48h, pd.DataFrame):
    predictions_df = res48h
else:
    raise RuntimeError("Unexpected format for res48h from model_armax_48h.fit")

# Save predictions, observations, and a combined file
out_path_multiindex = os.path.join(plot_dir, "armax_res48h.csv")
predictions_df.to_csv(out_path_multiindex)
print(f"\nPredictions with MultiIndex saved to {out_path_multiindex}")

observations_df = forty_eight_hourly_data.fc[["HC.f_future48h"]].loc[predictions_df.index]
...
combined_path = os.path.join(plot_dir, "armax_res48h_with_obs.csv")
combined_df.to_csv(combined_path)
print(f"Combined predictions and observations saved to {combined_path}")

# --- OLS Analysis for h=1 ---
horizon = 1
X_full, Y_full = model_armax_48h.transform_data(forty_eight_hourly_data, use_recursive_pars=False)
...
y_target = forty_eight_hourly_data[y_target_col].copy()
...
print("\n=== OLS Results for 48h-Aggregated ARMAX Model ===")
...
with open(results_file_path, "w") as f:
    f.write("=== OLS Results for 48h-Aggregated ARMAX Model ===\n")
...
pred_col = ("HC.f_future48h", horizon)
...
observations_h1 = forty_eight_hourly_data[[obs_col]].copy()
...
ax.plot(valid_h1.index, valid_h1["Obs"], label="Actual 48h Sum", color="black", linewidth=2.0)
ax.plot(valid_h1.index, valid_h1["Pred"], label=f"Pred 48h Sum (k={horizon})", color="red", linestyle="-", linewidth=0.8)
ax.set_title(f"Online ARMAX 48-Hour Forecast (k={horizon}) - Post Burn-in")
ax.legend()
ax.grid(True)
plt.tight_layout()
fig_path = os.path.join(plot_dir, f"armax_forecast_48h_{horizon}_step_post_burn_in.png")
...
if not valid_h1.empty:
    ...
    plt.tight_layout()
    diag_fig_path = os.path.join(plot_dir, f"armax_diagnostics_48h_{horizon}_step.png")
