...
#
# 1. Prepare 6-hourly aggregated data
#
...
# Create the final DataFrame for the 6-hour model
six_hourly_data = c.new_fc()

# Feature Engineering: Create rolling features for 6-hour aggregation
# Input feature: Sum of the PAST 6 hours of HC.f
...
# Target variable: Sum of the NEXT 6 hours of HC.f
...
model_armax_6h = ForecastModel(
    ...
    kseq=tuple(range(1, 9)) # Predict 8 steps ahead (8 * 6h = 48h forecast horizon)
)

# Define model output and inputs
model_armax_6h.add_outputs("HC.f_future6h")
model_armax_6h.add_inputs(
    "HC.f_past6h",
    "Taobs_smooth"
)

# Fit the model
res6h = model_armax_6h.fit(six_hourly_data.copy())
print("ARMAX 6h model fitting completed.")

# --- Save Raw Predictions to CSV ---
plot_dir = "../7_results/Case_1/2_armax/res6"
os.makedirs(plot_dir, exist_ok=True)

# Ensure predictions are in the expected DataFrame format
if isinstance(res6h, tuple) and len(res6h) > 0 and isinstance(res6h[0], pd.DataFrame):
    predictions_df = res6h[0]
elif isinstance(res6h, pd.DataFrame):
    predictions_df = res6h
else:
    raise RuntimeError("Unexpected format for res6h from model_armax_6h.fit")

# Save predictions, observations, and a combined file
out_path_multiindex = os.path.join(plot_dir, "armax_res6h.csv")
...
combined_path = os.path.join(plot_dir, "armax_res6h_with_obs.csv")
combined_df.to_csv(combined_path)
print(f"Combined predictions and observations saved to {combined_path}")

# --- OLS Analysis for h=1 ---
horizon = 1
X_full, Y_full = model_armax_6h.transform_data(six_hourly_data, use_recursive_pars=False)
...
print("\n=== OLS Results for 6h-Aggregated ARMAX Model ===")
...
with open(results_file_path, "w") as f:
    f.write("=== OLS Results for 6h-Aggregated ARMAX Model ===\n")
...
pred_col = ("HC.f_future6h", horizon)
...
ax.plot(valid_h1.index, valid_h1["Obs"], label="Actual 6h Sum", color="black", linewidth=2.0)
ax.plot(valid_h1.index, valid_h1["Pred"], label=f"Pred 6h Sum (k={horizon})", color="red", linestyle="-", linewidth=0.8)
ax.set_title(f"Online ARMAX 6-Hour Forecast (k={horizon}) - Post Burn-in")
ax.legend()
ax.grid(True)
plt.tight_layout()
fig_path = os.path.join(plot_dir, f"armax_forecast_6h_{horizon}_step_post_burn_in.png")
...
if not valid_h1.empty:
    ...
    plt.tight_layout()
    diag_fig_path = os.path.join(plot_dir, f"armax_diagnostics_6h_{horizon}_step.png")
