...
# Create a new feature: the sum of the PAST 6 hours.
# The value at index 't' is the sum of HC.f from t-5 to t.
# We use min_periods=6 to ensure we only have sums over full windows.
...
# Create the TARGET variable: the sum of the NEXT 6 hours
# This is a rolling sum of the NEXT 6 hours (t+1 to t+6)
sum_target_6h = hourly_data[("HC.f", "NA")].shift(-6).rolling(window=6).sum()
...
#
model6h = ForecastModel(RLS, estimate_variance=False, kseq=tuple(range(1, 9)))

# The output is the forward-looking sum
model6h.add_outputs("HC.f_future6h")

# ##-- NEW: STEP 2 - MODEL DEFINITION --##
# Use the new backward-rolling sum feature as input
model6h.add_inputs(
    # AR1: The value of HC.f_past6h from t-1. This is the sum over |t-6..t-1|
    ar1_past_sum=AR("HC.f_past6h", order=1),

    # AR2: The value of HC.f_past6h from t-2. This is the sum over |t-7..t-2|
    ar2_past_sum=AR("HC.f_past6h", order=2),

    ar12_past_sum=AR("HC.f_past6h", order=12),

    ar24_past_sum=AR("HC.f_past6h", order=24),
...
model6h.update_params(rls_lambda=0.995)

# Fit the model on the new data
res6h = model6h.fit(six_hourly_data)


#
# 3. Save raw predictions to CSV
#
plot_dir = "../7_results/Case_1/1_rls/res6"
os.makedirs(plot_dir, exist_ok=True)

if isinstance(res6h, ForecastTuple) and len(res6h) > 0 and isinstance(res6h[0], pd.DataFrame):
    predictions_df = res6h[0]
elif isinstance(res6h, pd.DataFrame):
    predictions_df = res6h
else:
    raise RuntimeError("Unexpected format for res6h from model6h.fit")

out_path_multiindex = os.path.join(plot_dir, "rls_res6h.csv")
predictions_df.to_csv(out_path_multiindex)
print(f"\nPredictions with MultiIndex saved to {out_path_multiindex}")

observations_df = six_hourly_data.fc[["HC.f_future6h"]].loc[predictions_df.index]
...
combined_path = os.path.join(plot_dir, "rls_res6h_with_obs.csv")
...
y_target_col = ("HC.f_future6h", "NA")
y_target = six_hourly_data[y_target_col].copy()
...
print("\n=== OLS Results for 6h-Aggregated Model (Rolling Inputs) ===")
print(f"AIC: {ols_model.aic:.4f}")
print(f"BIC: {ols_model.bic:.4f}")
print("\nP-values of regressors:")
print(ols_model.pvalues)

results_file_path = os.path.join(plot_dir, "AIC_BIC_pv_6h.txt")
with open(results_file_path, "w") as f:
    f.write("=== OLS Results for 6h-Aggregated Model (Rolling Inputs) ===\n")
...
obs_col = ("HC.f_future6h", "NA")
observations_h1 = six_hourly_data[[obs_col]].copy()
...
ax.plot(merged_h1.index, merged_h1["Obs"], label="Actual 6h Sum", color="black", linewidth=2.0)
ax.plot(merged_h1.index, merged_h1["Pred"], label=f"Pred 6h Sum (k={horizon})", color="red", linestyle="-", linewidth=0.8)
ax.set_title(f"Online RLS 6-Hour Forecast with Rolling Inputs (k={horizon})")
ax.legend(); ax.grid(True)
plt.tight_layout()
fig.savefig(os.path.join(plot_dir, f"rls_forecast_6h_{horizon}_step.png"), dpi=300, bbox_inches="tight")
plt.close(fig)
print(f"Forecast plot saved to {os.path.join(plot_dir, f'rls_forecast_6h_{horizon}_step.png')}")
...
if not valid_h1.empty:
    ...
    plt.tight_layout()
    fig.savefig(os.path.join(plot_dir, f"rls_diagnostics_6h_{horizon}_step.png"), dpi=300, bbox_inches="tight")
    plt.close(fig)
    print(f"Diagnostic plots saved for k={horizon} → {os.path.join(plot_dir, f'rls_diagnostics_6h_{horizon}_step.png')}")
