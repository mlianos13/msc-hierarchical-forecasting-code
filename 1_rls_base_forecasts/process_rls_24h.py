...
twentyfour_hourly_data = c.new_fc()

# ##-- STEP 1: FEATURE ENGINEERING --##
# Create a new feature: the sum of the PAST 24 hours.
twentyfour_hourly_data[("HC.f_past24h", "NA")] = hourly_data[("HC.f", "NA")].rolling(window=24, min_periods=24).sum()
# Create a smoothed temperature feature
twentyfour_hourly_data[("Taobs_smooth", "NA")] = hourly_data[("Taobs", "NA")].rolling(window=24, min_periods=24).mean()

# Create the TARGET variable: the sum of the NEXT 24 hours
sum_target_24h = hourly_data[("HC.f", "NA")].shift(-24).rolling(window=24).sum()
twentyfour_hourly_data[("HC.f_future24h", "NA")] = sum_target_24h

# Drop all rows with NaN values which were created by the rolling windows
twentyfour_hourly_data.dropna(inplace=True)


#
# 2. Model setup with new inputs and target
#
model24h = ForecastModel(RLS, estimate_variance=False, kseq=tuple(range(1, 3))) # Predict 2 horizons (k=1, k=2)

# The output remains the forward-looking sum
model24h.add_outputs("HC.f_future24h")

# Use the new backward-rolling sum features as input
model24h.add_inputs(
    ar1_past_sum=AR("HC.f_past24h", order=1),
    ar2_past_sum=AR("HC.f_past24h", order=2),
    ar12_past_sum=AR("HC.f_past24h", order=12),
    ar24_past_sum=AR("HC.f_past24h", order=24),
    ta_smooth=AR("Taobs_smooth", order=1),
    mu=One()
)
model24h.update_params(rls_lambda=0.995)

# ##-- Fit the model on the HOURLY (non-subsampled) data --##
res24h = model24h.fit(twentyfour_hourly_data)


#
# 3. Save raw predictions to CSV
#
plot_dir = "../7_results/Case_1/1_rls/res24"
os.makedirs(plot_dir, exist_ok=True)

if isinstance(res24h, ForecastTuple):
    predictions_df = res24h[0]
else:
    predictions_df = res24h

out_path_multiindex = os.path.join(plot_dir, "rls_res24h.csv")
predictions_df.to_csv(out_path_multiindex)
print(f"\nPredictions with MultiIndex saved to {out_path_multiindex}")

observations_df = twentyfour_hourly_data.fc[["HC.f_future24h"]].loc[predictions_df.index]
...
combined_path = os.path.join(plot_dir, "rls_res24h_with_obs.csv")
combined_df.to_csv(combined_path)
print(f"Combined predictions and observations saved to {combined_path}")


#
# 4. Compute AIC, BIC, and p-values for h = 1 via OLS
#
horizon = 1
X_full, Y_full = model24h.transform_data(twentyfour_hourly_data, use_recursive_pars=False)
...
y_target = twentyfour_hourly_data[y_target_col].copy()
...
print("\n=== OLS Results for 24h-Aggregated Model (Rolling Inputs, Hourly Update) ===")
...
with open(results_file_path, "w") as f:
    f.write("=== OLS Results for 24h-Aggregated Model (Rolling Inputs, Hourly Update) ===\n")
...
observations_h1 = twentyfour_hourly_data[[obs_col]].copy()
...
ax.plot(merged_h1.index, merged_h1["Obs"], label="Actual 24h Sum", color="black", linewidth=2.0)
ax.plot(merged_h1.index, merged_h1["Pred"], label=f"Pred 24h Sum (k={horizon})", color="red", linestyle="-", linewidth=0.8)
ax.set_title(f"Online RLS 24-Hour Forecast with Rolling Inputs (Hourly Update, k={horizon})")
ax.legend(); ax.grid(True)
plt.tight_layout()
fig.savefig(os.path.join(plot_dir, f"rls_forecast_24h_{horizon}_step.png"), dpi=300, bbox_inches="tight")
plt.close(fig)
print(f"Forecast plot saved to {os.path.join(plot_dir, f'rls_forecast_24h_{horizon}_step.png')}")
...
if not valid_h1.empty:
    ...
    plt.tight_layout()
    fig.savefig(os.path.join(plot_dir, f"rls_diagnostics_24h_{horizon}_step.png"), dpi=300, bbox_inches="tight")
    plt.close(fig)
    print(f"Diagnostic plots saved for k={horizon} → {os.path.join(plot_dir, f'rls_diagnostics_24h_{horizon}_step.png')}")
