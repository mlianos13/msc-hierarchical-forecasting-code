...
# Create a new feature: the sum of the PAST 12 hours.
# The value at index 't' is the sum of HC.f from t-11 to t.
# We use min_periods=12 to ensure we only have sums over full windows.
...
# Create the TARGET variable: the sum of the NEXT 12 hours
# This is a rolling sum of the NEXT 12 hours (t+1 to t+12)
...
#
model12h = ForecastModel(RLS, estimate_variance=False, kseq=tuple(range(1, 5)))

# The output is the forward-looking sum
model12h.add_outputs("HC.f_future12h")

# ##-- NEW: STEP 2 - MODEL DEFINITION --##
# Use the new backward-rolling sum feature as input
model12h.add_inputs(
    # AR1: The value of HC.f_past12h from t-1. This is the sum over |t-12..t-1|
    ar1_past_sum=AR("HC.f_past12h", order=1),

    # AR2: The value of HC.f_past12h from t-2. This is the sum over |t-13..t-2|
    ar2_past_sum=AR("HC.f_past12h", order=2),

    ar12_past_sum=AR("HC.f_past12h", order=12),

    ar24_past_sum=AR("HC.f_past12h", order=24),
...
model12h.update_params(rls_lambda=0.995)

# Fit the model on the new data
res12h = model12h.fit(twelve_hourly_data)


#
# 3. Save raw predictions to CSV
#
plot_dir = "../7_results/Case_1/1_rls/res12"
os.makedirs(plot_dir, exist_ok=True)

if isinstance(res12h, ForecastTuple) and len(res12h) > 0 and isinstance(res12h[0], pd.DataFrame):
    predictions_df = res12h[0]
elif isinstance(res12h, pd.DataFrame):
    predictions_df = res12h
else:
    raise RuntimeError("Unexpected format for res12h from model12h.fit")

out_path_multiindex = os.path.join(plot_dir, "rls_res12h.csv")
...
combined_path = os.path.join(plot_dir, "rls_res12h_with_obs.csv")
...
#
horizon = 1
X_full, Y_full = model12h.transform_data(twelve_hourly_data, use_recursive_pars=False)
...
print("\n=== OLS Results for 12h-Aggregated Model (Rolling Inputs) ===")
...
with open(results_file_path, "w") as f:
    f.write("=== OLS Results for 12h-Aggregated Model (Rolling Inputs) ===\n")
...
ax.plot(merged_h1.index, merged_h1["Obs"], label="Actual 12h Sum", color="black", linewidth=2.0)
ax.plot(merged_h1.index, merged_h1["Pred"], label=f"Pred 12h Sum (k={horizon})", color="red", linestyle="-", linewidth=0.8)
ax.set_title(f"Online RLS 12-Hour Forecast with Rolling Inputs (k={horizon})")
ax.legend(); ax.grid(True)
plt.tight_layout()
fig.savefig(os.path.join(plot_dir, f"rls_forecast_12h_{horizon}_step.png"), dpi=300, bbox_inches="tight")
plt.close(fig)
print(f"Forecast plot saved to {os.path.join(plot_dir, f'rls_forecast_12h_{horizon}_step.png')}")
...
if not valid_h1.empty:
    ...
    plt.tight_layout()
    fig.savefig(os.path.join(plot_dir, f"rls_diagnostics_12h_{horizon}_step.png"), dpi=300, bbox_inches="tight")
    plt.close(fig)
    print(f"Diagnostic plots saved for k={horizon} → {os.path.join(plot_dir, f'rls_diagnostics_12h_{horizon}_step.png')}")
